...
function Chloex:Window(GuiConfig)
    ...
    Close.Activated:Connect(function()
        ...
        Dialog.Image = "rbxassetid://115390576092568"
        ...
        Gradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0.0, Color3.fromRGB(255, 165, 0)),
            ColorSequenceKeypoint.new(0.25, Color3.fromRGB(255, 255, 255)),
            ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 140, 0)),
            ColorSequenceKeypoint.new(0.75, Color3.fromRGB(255, 255, 255)),
            ColorSequenceKeypoint.new(1.0, Color3.fromRGB(255, 165, 0))
        })
    ...
    UIStroke14.Color = Color3.fromRGB(255, 165, 12)
    UIStroke14.Thickness = 2.5
    UIStroke14.Transparency = 0.8
    UIStroke14.Parent = DropdownSelect

    DropdownSelectReal.AnchorPoint = Vector2.new(0.5, 0.5)
    DropdownSelectReal.BackgroundColor3 = Color3.fromRGB(98, 55, 0)
            ...
            function Items:AddSlider(SliderConfig)
                ...
                SliderFrame.InputEnded:Connect(function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                        Dragging = false
                        if type(SliderConfig.Callback) == "function" then
                            local ok, err = pcall(SliderConfig.Callback, SliderFunc.Value)
                            if not ok then warn("Slider Callback error:", err) end
                        end
                        TweenService:Create(
                            SliderCircle,
            ...
            function Items:AddDropdown(DropdownConfig)
                ...
                function DropdownFunc:Set(Value)
                    ...
                    if type(DropdownConfig.Callback) == "function" then
                        local ok, err
                        if DropdownConfig.Multi then
                            ok, err = pcall(DropdownConfig.Callback, DropdownFunc.Value)
                        else
                            local str = (DropdownFunc.Value ~= nil) and tostring(DropdownFunc.Value) or ""
                            ok, err = pcall(DropdownConfig.Callback, str)
                        end
